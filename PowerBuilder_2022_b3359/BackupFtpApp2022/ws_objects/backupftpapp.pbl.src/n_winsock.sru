...
Function long GetNameInfo ( &
	sockaddr_in pSockaddr, &
	ulong SockaddrLength, &
	Ref string pNodeBuffer, &
	ulong NodeBufferSize, &
	Ref string pServiceBuffer, &
	ulong ServiceBufferSize, &
	long Flags &
	) Library "ws2_32.dll" Alias For "GetNameInfoW"
...
Indirect Long IPVersion {of_SetIPVersion(*value),of_GetIPVersion()}
Indirect Long ConnTimeout {of_SetConnTimeout(*value),of_GetConnTimeout()}
Indirect Long RecvTimeout {of_SetRecvTimeout(*value),of_GetRecvTimeout()}
Indirect Long SendTimeout {of_SetSendTimeout(*value),of_GetSendTimeout()}
...
public function string of_getnameinfo (string as_ipaddress)
public function long of_getipversion ()
public function long of_getlasterrornum ()
end prototypes
...
//
// PURPOSE:		This function gets the last error number.
//
// RETURN:     The error number associated with the most recent function call.
//
// DATE			PROG/ID		DESCRIPTION OF CHANGE / REASON
// ----------	--------		-----------------------------------------------------
// 02/18/2021	RolandS		Initial coding
// -----------------------------------------------------------------------------
...
public function string of_getnameinfo (string as_ipaddress);// -----------------------------------------------------------------------------
// FUNCTION:	of_GetNameInfo
//
// PURPOSE:		This function gets the host name associated with an IP addresses.
//
// ARGUMENTS:  as_ipaddress	- The IP address to get host name for.
//
// RETURN:     Host Name
//
// DATE			PROG/ID		DESCRIPTION OF CHANGE / REASON
// ----------	--------		-----------------------------------------------------
// 01/12/2022	RolandS		Initial coding
// -----------------------------------------------------------------------------

sockaddr_in NameAddr4

Constant ULong NI_MAXSERV = 32
Constant ULong NI_MAXHOST = 1025
Constant Long NI_NUMERICSERV = 8
Constant UInt NamePort = 27015

Long iResult
String ls_HostName, ls_ServInfo
ULong StructSize

ls_HostName = Space(NI_MAXHOST)
ls_ServInfo = Space(NI_MAXSERV)

try 
	PopulateError(0, "GetNameInfo")
	StructSize = 16
	NameAddr4.sin_family = AF_INET
	NameAddr4.sin_addr.s_addr = inet_addr(as_ipaddress)
	NameAddr4.sin_port = htons(NamePort)

	iResult = GetNameInfo(NameAddr4, StructSize, ls_HostName, &
					NI_MAXHOST, ls_ServInfo, NI_MAXSERV, NI_NUMERICSERV)
...
public function long of_getipversion ();// -----------------------------------------------------------------------------
// FUNCTION:	of_GetIPVersion
//
// PURPOSE:		This function gets the current IP Version.
//
// RETURN:     AF_INET  = 2
//					AF_INET6 = 23
//
